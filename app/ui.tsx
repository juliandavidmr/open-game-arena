...
const resources = {
	...
	es: {
		translation: {
			...
			empty: "Todavía no hay Matches completadas",
			load: "Cargar más",
			result: "Resultado",
			cause: "Causa final",
			duration: "Duración",
...
export function Controls({ language }: { language: string }) {
	const { i18n } = useTranslation();
	useEffect(() => {
		void i18n.changeLanguage(language);
	}, [language, i18n]);
	function lang(v: string) {
		document.cookie = `oga-language=${v};path=/;max-age=31536000;samesite=lax`;
		void i18n.changeLanguage(v);
		location.reload();
	}
	function theme(v: string) {
		document.documentElement.dataset.theme = v;
		localStorage.setItem("oga-theme", v);
	}
	return (
...
export function ArenaHome({ language }: { language: string }) {
	...
	async function more() {
		const x = await fetch(
			`/api/matches?cursor=${encodeURIComponent(cursor!)}`,
		).then((r) => r.json());
	...
	return (
		<>
			...
			<main>
				<section className="hero min-h-[65vh] bg-base-200">
					<div className="hero-content text-center">
						<div className="max-w-2xl">
							<div className="badge badge-primary mb-5">{t("tagline")}</div>
							<h1 className="text-5xl md:text-7xl font-black tracking-tight">
								OPEN GAME
								<br />
								<span className="text-primary">ARENA</span>
							</h1>
							<p className="py-6 text-xl">{t("claim")}</p>
							<button
							...
							<p className="mt-8 opacity-60">
								{t("alternative")}: <code className="kbd">/mcp</code>
							</p>
						</div>
					</div>
				</section>
				<section className="max-w-6xl mx-auto p-6 md:py-16">
					...
					{!items.length ? (
						<div className="alert">{t("empty")}</div>
					) : (
