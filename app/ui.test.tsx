...
describe("ArenaHome", () => {
	it("hides the Completed Matches directory when there are no items", async () => {
		...
		expect(
			screen.getByRole("button", { name: "Launch Agent Battle" }),
		).toBeTruthy();
		expect(
			screen.getByRole("link", { name: "/mcp" }).getAttribute("href"),
		).toBe("/mcp");
		...
		expect(
			screen.getByRole("heading", { name: "Create the arena" }),
		).toBeTruthy();
		expect(
			screen.getByRole("heading", { name: "Deploy the contenders" }),
		).toBeTruthy();
		expect(
			screen.getByRole("heading", { name: "Watch every move" }),
		).toBeTruthy();
		await waitFor(() => expect(fetchMock).toHaveBeenCalledWith("/api/matches"));
		expect(
			screen.queryByRole("heading", { name: "Completed Matches" }),
		).toBeNull();
	});
	...
	it("shows the Completed Matches directory when the API returns an item", async () => {
		...
		expect(
			document.querySelector('a[href="/match/finished-match"]'),
		).not.toBeNull();
	});
});
